...
export default function DashboardActionHint({ to, icon, title, description, badge, tone = "neutral" }) {
    return (
        <Link
            to={to}
            aria-label={typeof title === "string" ? title : "Open dashboard action"}
            className="group block rounded-3xl border border-(--border-subtle) bg-(--surface-primary) p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-(--border-default) hover:shadow-(--shadow-sm)"
        >
